...
export default function Leccion({ questions, onComplete }: LeccionProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full p-4 sm:p-6 lg:p-8 flex flex-col min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 transition-colors">
      {/* Progress Bar */}
      <div className="mb-8">
      ...
      <div className="flex-grow flex flex-col relative">
        <AnimatePresence mode="wait">
          ...
          >
            <h2 className="text-2xl sm:text-3xl font-semibold text-slate-800 dark:text-white mb-8 leading-tight">
            ...
            <div className="space-y-4">
              {currentQuestion.options.map((option, index) => {
                ...
                let optionStyle = 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 hover:shadow-md';
                let icon = null;

                if (isAnswered) {
                  if (option.isCorrect) {
                    optionStyle = 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-500 text-emerald-900 dark:text-emerald-100 shadow-sm';
                    icon = <CheckCircle2 className="text-emerald-500 shrink-0" size={24} />;
                  } else if (isSelected && !option.isCorrect) {
                    optionStyle = 'bg-rose-50 dark:bg-rose-900/30 border-rose-500 text-rose-900 dark:text-rose-100 shadow-sm';
                    icon = <XCircle className="text-rose-500 shrink-0" size={24} />;
                  } else {
                    optionStyle = 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 opacity-60 grayscale';
                  }
                }
            ...
            <AnimatePresence>
              {isAnswered && (
                <motion.div
                  ...
                  className="bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 p-5 rounded-2xl"
                >
